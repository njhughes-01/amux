#!/usr/bin/env bash
# hook-report.sh may recover a missing $AMUX_SESSION from tmux only when the
# hook itself runs inside a tmux pane. Outside tmux, `tmux display-message`
# answers for the most recently used session, so an operator's plain Claude
# session was reported as (and pinned to) an unrelated lane.
set -Eeuo pipefail
